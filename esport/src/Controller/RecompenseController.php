...
class RecompenseController extends AbstractController
{
    ...
    public function index(Request $request, RecompenseRepository $recompenseRepository): Response
    {
        ...
        // Récupérer TOUTES les récompenses pour les diagrammes
        $allRecompenses = $recompenseRepository->findAll();
        
        // Récupérer les récompenses filtrées pour le tableau
        $recompenses = $recompenseRepository->searchByNameAndType($searchName, $searchType, $sort);

        // Calculer les statistiques sur les récompenses filtrées
        $minClassement = null;
        $maxClassement = null;
    ...
    public function getModalForm(Request $request, EntityManagerInterface $entityManager, ValidatorInterface $validator): JsonResponse
    {
        $action = $request->query->get('action'); // 'create' ou 'edit'
        $id = $request->query->get('id');

        $recompense = $action === 'edit' ? $entityManager->getRepository(Recompense::class)->find($id) : new Recompense();
        ...
        if ($request->isMethod('POST')) {
            ...
            if ($form->isSubmitted() && $form->isValid()) {
                ...
                if (count($errors) === 0) {
                    ...
                    $entityManager->flush();
                    return new JsonResponse(['success' => true, 'message' => 'Récompense ' . ($action === 'edit' ? 'modifiée' : 'ajoutée') . ' avec succès']);
                }
            }
        }
    ...
    public function exportPdf(RecompenseRepository $recompenseRepository, PdfGenerator $pdfGenerator): Response
    {
