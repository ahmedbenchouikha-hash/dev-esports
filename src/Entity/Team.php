...
class Team
{
    ...
    public function addTournamentRegistration(TournamentRegistration $registration): static
    {
        if (!$this->tournamentRegistrations->contains($registration)) {
            $this->tournamentRegistrations->add($registration);
            $registration->setTeam($this);
        }
        return $this;
    }

    public function removeTournamentRegistration(TournamentRegistration $registration): static
    {
        if ($this->tournamentRegistrations->removeElement($registration)) {
    ...
    {
        return $this->tournamentRegistrations->exists(fn(int $key, TournamentRegistration $reg) => 
            $reg->getTournament() === $tournament
        );
    }

    public function getApprovedTournamentRegistrations(): Collection
    {
        return $this->tournamentRegistrations->filter(fn(TournamentRegistration $reg) => $reg->isApproved());
    }

    public function getPendingTournamentRegistrations(): Collection
    {
        return $this->tournamentRegistrations->filter(fn(TournamentRegistration $reg) => $reg->isPending());
    }
