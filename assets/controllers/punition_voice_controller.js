...
export default class extends Controller {
    static targets = ['button', 'status', 'transcript', 'answer', 'manual'];
    static values = {
        endpoint: String,
    };
    ...
    connect() {
        ...
        this.recognition.onresult = async (event) => {
            ...
            if (preview !== '' && this.hasTranscriptTarget) {
    ...
    async toggle() {
        if (!this.recognition) {
            this.updateStatus('Micro indisponible. Utilisez le champ texte ci-dessous.');
            return;
        }
    ...
    async sendManual(event) {
        ...
        const message = (this.manualTarget.value || '').trim();
        if (message === '') {
            this.updateStatus('Veuillez écrire un message avant d’envoyer.');
            return;
        }
        ...
        if (this.hasTranscriptTarget) {
            this.transcriptTarget.textContent = message;
        }
